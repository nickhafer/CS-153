...
"-----------------------------------------------------------
🌿 Sometimes not all the activities/parks/etc. are included in the recreation.gov database, so we're supplementing with AI to give a more complete response! The following {num_supplementary} results are additional suggestions generated by AI:"
...
Also, add friendly nature-themed emojis (🌲, 🏞️, 🌄, 🏕️, 🚶‍♀️, 🌿, 🌅, 🏔️, etc.) throughout your response to make it more engaging. Add an emoji at the start of your response and before each numbered result.

At the very end of your response, add a brief reminder about the benefits of spending time outdoors. Choose ONE of these benefits or create a similar one:
...
"""
            ...
            # Add instruction for nature emojis even if there are no supplementary results
            summarize_prompt += """
\n\nIMPORTANT: Add friendly nature-themed emojis (🌲, 🏞️, 🌄, 🏕️, 🚶‍♀️, 🌿, 🌅, 🏔️, etc.) throughout your response to make it more engaging. Add an emoji at the start of your response and before each numbered result.
...
"""
        ...
        for attempt in range(2):
            ...
            if (has_numbered_list or has_bullet_points) and num_results > 1:
                ...
                if supplementary_results and not any(phrase in response_text.lower() for phrase in [
                    "supplementing with ai", 
                    "not found in the recreation.gov database", 
                    "additional suggestions generated by ai",
                    "-----------------------------------------------------------"
                ]):
                ...
                # Add a nature emoji at the beginning if there isn't one already
                ...
                if not any(benefit in response_text[-200:] for benefit in ["time in nature", "time outdoors", "outdoor activity", "green spaces", "natural light", "time in the outdoors", "spending time outside"]):
                    # Choose a random benefit to add
                    benefits = [
                        "\n\n🌳 Remember, just 20 minutes in nature can significantly lower stress hormone levels!",
                        "\n\n🏞️ Studies show that time spent outdoors can improve your concentration and mental clarity.",
                        "\n\n🌄 Spending time in nature has been linked to improved immune function and overall health.",
                    ...
                    response_text += benefits[hash(response_text + str(time.time())) % len(benefits)]
            ...
            if num_results == 1 or attempt == 1:
                if num_results > 1:
                    ...
                    response_text = f"🏞️ Here are {num_results} recreational options I found:\n\n" + response_text
            ...
            summarize_prompt += f"\n\nYour previous response did not clearly include all {num_results} results. Please list EACH result separately with a number (1., 2., etc.) and include ALL {num_results} results. Add a nature emoji (🌲, 🏞️, etc.) before each numbered result. Also, be sure to include a reminder about the benefits of spending time outdoors at the very end of your response."
    ...
    async def run(self, message: discord.Message):
        ...
        try:
            ...
            if no_results:
                ...
                # Add a benefit reminder for no results too
                benefits = [
                    "\n\n🌳 While you're searching, remember that just 20 minutes in nature can significantly lower stress hormone levels!",
                    "\n\n🏞️ Did you know? Studies show that time spent outdoors can improve your concentration and mental clarity.",
                    "\n\n🌄 Even a short nature walk can boost your mood and energy levels. Keep exploring!",
                    "\n\n🌿 Looking forward to your outdoor adventure! Regular outdoor activity can help improve sleep quality and reduce insomnia.",
                    "\n\n🚶‍♀️ Keep searching for that perfect outdoor spot! Time in green spaces has been shown to reduce symptoms of anxiety and depression.",
                    "\n\n🌅 When you find the right outdoor activity, you'll also strengthen your connection with your local community and environment.",
                    "\n\n🏔️ Don't give up on finding the perfect outdoor spot! Exposure to natural light helps regulate your body's internal clock and improve mood."
                ]
                response_text += benefits[hash(response_text + str(time.time())) % len(benefits)]
            ...
            if len(response_text) > 1990:
                ...
                # Return as a single-item list for consistent handling
                return [response_text]
